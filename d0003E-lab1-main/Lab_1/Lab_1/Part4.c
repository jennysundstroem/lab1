...
void combine() {
	...
	uint16_t clockFactor = 8000000/512;
	...
	if(TCNT1 >= clockCount && blinkFlag == 0){						//before next prime number do blinking
			blinkFlag = 1;
			
			if((clockCount + clockFactor) > 0xFFFFF){
				clockCount = ((clockCount + clockFactor) - 0xFFFFF);
				
			}else{
				clockCount = (clockCount + clockFactor);
			}
			if (LCDDR8 != 0) {
					LCDDR8 = 0x0;
			}else {
